...
export function initScrollEngine(): () => void {
  ...
  function updatePhaseIndicator() {
    ...
    let currentPhase: { id: string; num: number; label: string } | null = null;
    const scrollCenter = state.scrollY + state.windowHeight * 0.4;
    ...
    if (currentPhase) {
      ...
      navPhaseDot!.textContent = String((currentPhase as { num: number }).num);
      navPhaseLabel!.textContent = (currentPhase as { label: string }).label;
    } else {
      navPhase.classList.remove('visible');
    }

    // Update active menu link
    const menuLinks = document.querySelectorAll<HTMLElement>('.nav__menu-link');
