 ...
 * Scroll-driven step progression with clickable pins on 3D model
 * Matches NRG "Build Your Data Center" interaction pattern
 */
...
(function () {
  ...
  // Each step has: phase, step number, label, description text,
  // camera state for 3D scene, and array of pins
  // ============================================
  ...
  function openCard(pinData, pinEl) {
  ...
  document.addEventListener('click', (e) => {
    if (cardOpen && !card.contains(e.target) && !e.target.closest('.pin')) {
      closeCard();
    }
  });
  ...
  function updateNav(step, stepIndex) {
    if (navPhaseDot) navPhaseDot.textContent = step.phase;
  ...
  function transitionCamera(cameraState) {
    // Bridge to scene3d.js via global API
