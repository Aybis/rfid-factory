 ...
 * Persistent Three.js scene that transitions between views as user scrolls
 * Each section has its own 3D camera angle and highlighted elements
 */
...
window.addEventListener('load', function () {
  ...
  let targetCamPos = new THREE.Vector3(65, 45, 65);
  ...
  // --- RFID Gate Portals ---
  function createGate(x, y, z, rotY) {
  ...
  for (let i = -23; i <= -17; i += 3)
    scene.add(box(1.5, 1.5, 0.1, mat.glass, i, 4, -9.05));
  ...
  function addWave(x, y, z) {
    ...
    ring.userData = { speed: 0.4 + Math.random() * 0.3, phase: Math.random() * Math.PI * 2, baseY: y };
    scene.add(ring);
    waves.push(ring);
  }
  ...
  const dataFlowLines = [];
  function addDataLine(x1, y1, z1, x2, y2, z2) {
  ...
  function createHighlightBox(w, h, d, x, y, z) {
  ...
  // ============================================
  // PUBLIC API — Called by pins.js
  // ============================================

  window.rfidScene = {
    transitionTo: function (cameraState) {
  ...
  function animate() {
    ...
    waves.forEach(w => {
      const p = ((t * w.userData.speed + w.userData.phase) % 1);
      const s = 0.3 + 2.7 * p;
      w.scale.set(s, s, 1);
      w.material.opacity = 0.6 * (1 - p);
      w.position.y = w.userData.baseY + p * 0.5;
    });

    // --- Animate server LEDs ---
    serverLeds.forEach(led => {
      led.material.emissiveIntensity = 0.2 + 0.8 * Math.abs(Math.sin(t * 3 + led.position.x * 5));
    });
    ...
    if (currentState === 'phase1') {
      scanPlane.material.opacity += (0.04 - scanPlane.material.opacity) * 0.05;
      const scanZ = -20 + ((t * 5) % 44);
      scanLineMesh.position.z = scanZ;
      scanLineMesh.material.opacity += (0.5 - scanLineMesh.material.opacity) * 0.05;
      hlWarehouse.material.opacity += (0.6 - hlWarehouse.material.opacity) * 0.05;
    } else {
      scanPlane.material.opacity += (0 - scanPlane.material.opacity) * 0.05;
      scanLineMesh.material.opacity += (0 - scanLineMesh.material.opacity) * 0.05;
